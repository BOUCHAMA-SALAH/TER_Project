#-------------------------------------------
# call: bash ./semi_skept.sh <some_instance.lp> <arg>
#-------------------------------------------
...
# compute the range maximal answer sets | select answer set | select answersets with <arg> in the range
    echo "#show out/1. undec(dummy964052wd)." | "$dirClingo" - "$dir"/heuristic_semi.lp $1 --heuristic=Domain --enum=domRec 0 2>/dev/null  | grep 'range' | sed 's/)/)./g; s/range//g' >  "$rangeList"
